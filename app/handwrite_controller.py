...
class RecognitionThread(QThread):
    ...
    def run(self):
        ...
        try:
            config = OCRConfig()
            ocr = self._create_ocr_adapter(config)
            self._configure_preprocessing(ocr, config)
            self._log_recognition_start()
            text = self._recognize_with_logging(ocr)
            text = self._handle_empty_result_with_fallback(text, ocr, config)
            self._emit_result(text)
        except Exception as e:
            self._handle_unexpected_error(e)

    def _create_ocr_adapter(self, config: OCRConfig) -> UnifiedOCRAdapter:
        """Створення та налаштування OCR адаптера з урахуванням LLM."""
        llm_config = config.get_llm_config()
        use_llm = llm_config.get('enabled', False)
        return UnifiedOCRAdapter(
    ...
    @staticmethod
    def _configure_preprocessing(ocr: UnifiedOCRAdapter, config: OCRConfig) -> None:
        """Встановлення конфігурації препроцесингу."""
        preprocessing_config = config.get_preprocessing_config()
        ocr.set_preprocessing_config(preprocessing_config)

    def _log_recognition_start(self) -> None:
        """Логування початку розпізнавання та оновлення прогресу."""
    ...
    def _recognize_with_logging(self, ocr: UnifiedOCRAdapter) -> str:
        """Виклик ocr.recognize() з детальним логуванням."""
        try:
            ...
            print(
                f"[Controller] ocr.recognize() завершено, результат: {result_len} символів",
                flush=True,
            )
            self.progress.emit(80, "Обробка результату...")
            logger.info(
                "[Controller] Результат від рушія '%s': %s символів",
                self.ocr_engine,
                result_len,
            )
            if text:
            ...
            print(
                f"[Controller] ✗ Помилка в ocr.recognize(): {recognize_error}",
                flush=True,
            )
            traceback_str = traceback.format_exc()
            print(f"[Controller] Traceback:\n{traceback_str}", flush=True)
            logger.error(
                "[Controller] ✗ Помилка в ocr.recognize(): %s",
                recognize_error,
                exc_info=True,
            )
            raise

    def _handle_empty_result_with_fallback(
        self, text: str, ocr: UnifiedOCRAdapter, config: OCRConfig
    ) -> str:
        """Обробка порожнього результату та fallback на інші рушії, якщо дозволено."""
        if text and text.strip():
            return text

        logger.warning(
            "[Controller] ⚠️ РЕЗУЛЬТАТ ПОРОЖНІЙ від рушія '%s'", self.ocr_engine
        )
        ...
        logger.warning(
            "[Controller] ⚠️ УВІМКНЕНО FALLBACK - спроба інших рушіїв"
        )
        available_engines = ocr.get_available_engines()
        logger.info(
            "[Controller] Доступні рушії для fallback: %s", available_engines
        )
        ...
        for fallback_engine in available_engines:
            ...
            try:
                logger.warning(
                    "[Controller] ⚠️ FALLBACK: Спроба рушія %s", fallback_engine
                )
                ...
                if fallback_text and fallback_text.strip():
                    logger.warning(
                        "[Controller] ⚠️ FALLBACK: Використовується результат "
                        "від %s замість %s",
                        fallback_engine,
                        self.ocr_engine,
                    )
                ...
                logger.error(
                    "[Controller] ✗ Fallback на %s не вдався: %s",
                    fallback_engine,
                    fallback_error,
                )
                continue
    ...
    def _emit_result(self, text: str) -> None:
        """Відправка фінального результату або помилки, якщо текст порожній."""
        if not text or not text.strip():
            self.error.emit(
                "Не вдалося розпізнати текст. Спробуйте інше зображення або змініть налаштування."
            )
        else:
            self.finished.emit(text)

    def _handle_unexpected_error(self, error: Exception) -> None:
        """Обробка неочікуваних помилок та логування в файл."""
        import traceback

        error_msg = f"Помилка при розпізнаванні: {error}"
        traceback_str = traceback.format_exc()
        ...
        try:
            with open("controller_error.log", "a", encoding="utf-8") as f:
                f.write(f"\n{'='*80}\n")
                f.write(f"Помилка при розпізнаванні: {error}\n")
                f.write(f"Рушій: {self.ocr_engine}, Мова: {self.language}\n")
                f.write(f"Traceback:\n{traceback_str}\n")
                f.write(f"{'='*80}\n")
        except Exception:
            pass
        self.error.emit(f"Помилка при розпізнаванні: {str(error)}")
